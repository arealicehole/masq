...
Masq is a portable image processing engine for **background removal** and **upscaling**. It uses a "shotgun" approach that fires 5 AI models in parallel for background removal, returning all results for user selection. Upscaling uses Lanczos resampling to preserve alpha/transparency (critical for DTF printing workflows).
...
- `upscale(image_bytes, scale=4, preserve_alpha=True)` - Lanczos upscaling
